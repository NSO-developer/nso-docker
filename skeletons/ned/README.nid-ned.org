...
** The test environment - testenv
   ...
   - =testenv-build=: recompile packages and run ~request packages reload~ in NSO
     - only packages in =packages/= will be rebuilt, included packages that are specified through manifests in the =includes/= directory, are not rebuilt.
       - as they might not ship with their source code, it might not even be possible
   - =testenv-clean=: for the running NSO container, run ~make clean~ for all packages in the =packages/= and =test-packages/= directory
