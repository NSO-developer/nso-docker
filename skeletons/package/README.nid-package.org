...
** The test environment - testenv
   ...
   - =testenv-dev-shell=: Get an interactive =bash= shell in a container using the -dev image. The container is attached to the network namespace and volumes of the =testnso= container.
   ...
   fast to start and complete a basic test. If a project contains multiple
   environments you can choose the =testenv= to start in two ways:
   1. Set the =TESTENV= environment variable when executing the =testenv= make
      targets from the project directory, like ~TESTENV=quick make
      testenv-start~. All =testenv-%= targets are proxied to
      the currently selected =testenv=.
   2. Execute the =testenv-%= targets in the =testenv= directory, like ~make -C
      testenvs/quick testenv-start~
