...
# These patches are mostly not idempotent and do not need to be idempotent as we
# always start over with the default ncs.conf on every startup.
...
# This is idempotent; we update the node if it exists or add it, if it does not
# already exist.
xmlstarlet edit --inplace -N x=http://tail-f.com/yang/tailf-ncs-config \
