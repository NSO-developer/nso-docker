...
*** Injecting ncs.conf through a persistent volume
    ...
    The normal configuration mangling will NOT be applied to the mounted ~/etc/ncs/ncs.conf~. It can be enabled to run by setting ~MANGLE_CONFIG=true~, for example:
...
*** Writing your own ncs.conf
    ...
    - use of customer Python-VM startup script that supports Python virtualenvs
