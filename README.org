...
*** Run for production
    ...
    This uses the ~--net=host~ option to let the container live in the hosts networking namespace. This means that it binds to the IP address of the (virtual) machine it is running on.
    #+BEGIN_SRC shell
      docker run -itd --name nso -v /data/nso:/ncs --net=host my-prod-image:12345
    #+END_SRC
** Configuration options
   ...
   |----------------------+---------+---------+-----------------------------------------------|
   | PAM                  | boolean | false   | Enable PAM instead of local auth in NCS (AAA) |
