...
* Mirroring
  ...
  Enable mirroring from an upstream by scheduling a CI job and setting the =CI_MODE= variable to =mirror=. In addition, you need to set a number of other variables for the mirroring functionality to work:
  ...
  - =GIT_SSH_PRIV_KEY=: a private SSH key to use for cloning of its own repository and pushing the updates
    - create a deploy key that has write privileges and enter the private key
    - you can use your own SSH key too but it is better to use a deploy key
